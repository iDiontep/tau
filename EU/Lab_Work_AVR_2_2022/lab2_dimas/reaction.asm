...
; Register Variables
;*******************
.def temp       =R16;регистр для хранения временных данных, исп. для промежуточного хранения данных при обращении к регистрам РВВ;
.def Random     =R15;буфер случайного числа
.def Thousands  =R18;
.def Hundreds   =R19;адреса символов для семисегментных индикаторов по таблице перекодировки
.def Tens       =R20;
.def Ones       =R21;
.def CountX     =R14;счетчика числа переполнений таймера Т0 при формировании псевдослучайной задержки  
.def Disp_Numb  =R23;указатель включенного интервала
.def Disp_Count =R12; счетчик обновления дисплея
.def Disp_CountH =R28
.def TimeH      =R17;счетчик переполнений Т0
.def TimeL      =R25;
.def Count3     =R13;счетчик для организации процедуры умножения на 16 логическим сдвигом влево на 4 разряда;
.def tempH      =R22;преобразованое  значение, хранящегося в регистрах TimeH:TimeL, в миллисекунды
.def tempH2     =R26;
.def temp2      =R27;
;*****************
.def Byte_fl    =R24;байт флагов
;------------------
.equ F_generate        =1;бит флага "Готовность" (бит 1 в байте Byte_fl)
;***************** 
; Constants
;*****************
.equ Val_dispCount=50;величина константы,опр. время вкл индикатора 
.equ Val_dispCountH=15
;***********************************
.cseg
.org $0000
rjmp Init
...
rjmp  IN_INT0; Переход на процедуру обработки прерывания по нажатию кнопки "Реакция" 
;----------------
...
rjmp  IN_T0ovf; Переход на процедуру обработки прерывания переполнение таймера-счетчика 
;--------------------
...
;***********************************
Init:													;Инициализация стека (Устанавливаем указатель в начало стекаЫ)
...
Init_B:	  ldi   temp,0b11111111;(PB0-PB2)-выходы управления семисегментными индикаторами + PB7-СИД
          out   DDRB,temp
          ldi   temp,0b00001000;выбраны сотни (1-й индикатор)
	      out   PORTB,temp

Init_C:	  ser   temp;  (PC0-PC7) - выходы, задающие значение на семисегментном индикаторе
	      out   DDRC,temp
;
Init_D:    ldi   temp,0b11111000;PD1,PD2-входы, остальные-выходы (Кнопка "Готовность" PD1,кнопка "Реакция" PD2(INT0), PD0 - генерация)
	      out   DDRD,temp
	      ldi   temp,0b00000110;Вкл подтяжка на кнопках.   
	      out   PORTD,temp;

; ---  Инициализация  таймера TCNT0
Init_T0:  ldi   temp,(1<<TOIE0);TOIE0-Timer/Counter0 Overflow Interrupt Enable (разрешается прерывание по переполнению таймера)
	      out   TIMSK,temp
...
;
	      ...
	      out   MCUCR,temp
	      ldi   temp,(1<<INT0);INT0: External Interrupt Request 0 Enable (разрешение прерывания)
	      out   GIMSK,temp
;
Init_R:   ldi   Disp_Count,Val_dispCount; загружаем в регистр время вкл индикатора
		  ldi   temp,Val_dispCountH
		  mov   Disp_CountH, temp
	      clr   Disp_Numb ; устанавливаем нулевое значение текущего индикатора	    	     
          clr   Byte_fl; сбрасываем байт флагов (Сигнализирует о возникновения некоторого события)
	      ldi   temp,1     ;любое число
	      mov   Random,temp;в регистр Random (1)
		  clr	Thousands;
	      clr    Hundreds; обнуление регистров значений индикаторов
          clr    Tens    ;(0)
          clr    Ones
...
;начало цикла
;==================================================
Start:    rcall  Display; вызов процедуры динамической индикации
          sbrc   Byte_fl,F_generate; проверяется состояние флага F_ready (если бит1=0 пропускаем start) устанавливается после нажатия готов
	      rjmp   Start     ;внутренний цикл
	      sbic   PIND,0    ;ждем нажатия кнопки "Generate" 
	      rjmp   Start
;Формирование псевдослучайного числа
Rand_st:  mov    temp,Random;Вычисляем следующее случайное число 
          add    Random,temp;умножаем на 9 сложением
...
WaitForStart:
          ...
          clr    Ones
	      set
          bld    Byte_fl,F_generate;Флаг F_ready (бит1) в байте Byte_fl установить в 1
	      rjmp   Start
;==================================================	   
; конец цикла
;==================================================	
...
Display:  subi  Disp_Count,0x01
          ...
          inc   Disp_Numb;выбираем следующий дисплей (от 0 до 3)
		  cpi   Disp_Numb,4
		  brne  Out_disp; если дисплей не последний (Z =0 ) переход
		  clr   Disp_Numb; если дисплей посдений обнуляем и опять идем с нулевого
          rjmp  Out_disp
...
;
          in    temp,PINB
		  in    temp2,PINB
		  andi  temp,0b00001111;маскируем биты выбора индик.PB2 - сотни,PB1-десятки,PB0 - един.
		  andi  temp2,0b10000000;маскируем бит светодиода (СИД)
		  lsr   temp
		  brcc  PC+2; Проверка переноса (перед сдвигом вправо PB0 = 1)
...
;===================================================
IN_INT0:   push   temp
           in     temp,SREG;сохранение регистра статуса в стек
		   push   temp 
;
           sbis   PIND,0;проверяем СИД
           rjmp   Cheat;если 0 (не горит), то кнопка "stop" была нажата=
           clr    temp
		   out    TCCR0,temp;остaнов Т0
		   in     TimeL,TCNT0;счетный регистр таймера Т0 TCNT0 сохраняется в регистре TimeL
		   ;В регистре TimeH уже храниться число переполнений таймера Т0 (см обр прерывания IN_T0ovf)
...
;цикл, который осуществляет 4 логических сдвига влево для числа, образованного регистрами tempH2:tempH:temp
           ldi    temp,4 
...
; 
DoneDividing:
           rcall  digitConvert;преобразует число в значения регистров Hundreds, Tens и Ones   		   		    
;		     
ex_INT0:   clt;сбрасываем пользовательский бит Т
		   bld    Byte_fl,F_ready;загрузить бит в регистре из флага Т
		   clr    TimeH   ;сброс счетчика переполнений Т0
		   clr    temp
		   out    TCCR0,temp;остaнов Т0
		   cbi    PORTB,7;выключаем СИД
           pop    temp         ;выход без разрешения глоб. прерывания
           out    SREG,temp; возвращаем регистр статуса из стека
		   pop    temp
           ret
;================================================= 
...
digitConvert:
			...
			clr   Tens  
			clr   Ones
;Конвертация осуществляется последовательным вычитанием сначала числа 100, а затем числа 10 из пары регистров TimeH:TimeL 
;Остаток записывается в регистр Ones.
FindThousands:
            subi  TimeL,1000
			sbci  TimeH,10
			brcs  FindHundreds
...
;===================================================
IN_T0ovf:  push   temp
           in     temp,SREG;SREG сохраняется в стеке
		   push   temp 
           sbic   PINB,7;проверяем СИД
           rjmp   LEDon;Если бит не равен нулю (светодиод горит), переходим на LEDon
		   dec    CountX
		   brne   ex_T0ovf
...
ex_T0ovf:  pop    temp         ;выход c разрешениtv глоб. прерывания
		   ...
		   pop    temp
           reti
;таймер находится в режиме подсчета времени реакции
LEDon:     inc    TimeH         ;инкрементируем ст байт
           cpi    TimeH,0x1F    ;проверяем на максимальное время   
		   brlo   ex_T0ovf
;                              превышение времеми > 999 мсек
;Если значение этого регистра меньше 0х1F, значит 999 мс еще не прошло и можно продолжать счет.
;Если же оно стало равно или больше 0х1F, значит время ожидания истекло
out_HI:    clt
		   bld    Byte_fl,F_generate
		   clr    temp
		   out    TCCR0,temp;остaнов Т0
;          ldi    Hundreds,13 ;"-"
;		   ldi    Tens,14     ;"H" 
