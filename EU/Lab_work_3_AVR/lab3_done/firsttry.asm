...
.def temp_H     =R17
; 
.def Number     =R18;регистр, используемый для хранения адреса символа по таблице перекодировки, отображаемого на семисегментном индикаторе "номер байта";
.def Hundreds   =R19;
.def Tens       =R20
.def Ones       =R21;регистры, используемые для хранения адреса символа по таблице перекодировки, отображаемого на семисегментных индикаторах
;
.def Disp_Numb  =R22;регистр, используемый для хранения номера активного семисегментного индикатора
.def Disp_Count =R23;регистр, используемый  в качестве счетчика времени активности каждого из семисегментных индикаторов при динамической индикации
;
.def res_a_op   =R8;буфер хранения промежуточных результатов при выполнении арифметических операций
...
.def c_sumREC   =R6;контр сумма прин. байт (без переноса)
.def c_sumTRAN  =R7;контр сумма переданных байт (без переноса)
.def n_ar_op    =R24 ;регистр, используемый в качестве счетчика числа циклов суммирования при выполнении операции умножения
;*******************
...
;----------------------------------------
.equ   VAL_TR  =3;константа, определяющая количество бит к кадре, передаваемом модулем USART;
.equ   VAL_REC =4;константа, определяющая количество бит к кадре, принимаемом модулем USART;
.equ   VAL_time =30;35 переполнений соответствуют 1 сек
;***************************************
;Variable
;***************************************
.DSEG ;буферы приема и передачи хранятся в ОЗУ (SRAM) 
;
...
rjmp  Time_OUT; Прерывание по совпадению таймера 
...
;
;INIT USART
       ldi   temp_L,0x3B;(Частота кв. 9,216 мГц,скорость обмена 19200),U2X0=0,
       ldi   temp_H,00 
	   out   UBRRL,temp_L;задаем скорость передачи
	   out   UBRRH,temp_H
	   ldi   temp_L,(1<<RXEN)|(1<<RXCIE); разрешение прерывания по приему
	   out   UCSRB,temp_L
	   ldi   temp_L,(1<<URSEL)|(1<<UCSZ0)|(1<<UCSZ1)|(1<<UPM1)|(1<<UPM0);UCSZ2=0,UCSZ1=1,UCSZ0=1 - 8 bit;1 stop bit,
	   out   UCSRC,temp_L;
...
;                             No clock source,CS42,CS41,CS40=000
       ...
       out   OCR1AH,temp_H
	   out   OCR1AL,temp_L
;Инициализация динамической индикации семисегментные индикаторы
	   ...
	   clr   Time
	   ldi    Number,1
;Обнуление SRAM (буфер хранение переданных и принятых данных)
...
Start: rcall  Display
	   ...
	   rjmp   Receive
;Disab_Key активен в момент формирования защитного интервала при подавлении дребезга контактов от кнопки "Просмотр"
       sbrc   Byte_fl,Disab_Key; 
	   rjmp   Start
       sbic   PinD,4;Проверка нажатия кнопки(проверяет состояние кнопки "Просмотр")
	   rjmp   Start
	   rcall  ch_pos_D;осуществляет смену текущего отображаемого байта	   
	   rjmp   Start       
Receive: 
       sbi    PORTB,CID
	   rcall  Wr_D_rec;подпрограмма записи принятых данных в буфер для отображения на семисегментных индикаторах
       rcall  arifm_op;подпрограмма выполнения арифметической операции
       rcall  pre_date;подпрограмма подготовки данных для передачи модулем USART 
       rcall  Wr_D_tr;подпрограмма записи передаваемых данных в буфер для отображения на семисегментных индикаторах
Wait:  sbis   PIND,ENV;Проверка нажатия кнопки(проверяет состояние кнопки "Ответ")
       rjmp   Trans
       rcall  Display; дин. индикация
       sbrc   Byte_fl,Disab_Key; 
	   rjmp   Wait
...
Wait_tr: sbrs   Byte_fl,F_trans; ожидание окончания передачи (последний байт)
       rjmp   Wait_tr
	   clt
	   bld    Byte_fl,F_trans
       rjmp   Start
...
;==================================================
Wr_D_rec: ;Подпрограмма записи принятых данных в буфер дисплея (после проверки контрольной суммы)
;**************************************************
            ldi    YL,low(varBuf_Rxd)  ; Load Y register low буфер приема
            ldi    YH,high(varBuf_Rxd) ; Load Y register high буфер приема
            ldi    XL,low(varBuf_disp)  ; XL register low буфер данных,выводимых на дисплей
...
;Подпрограмма pre_date осуществляет вычисление контрольной суммы передаваемых данных и запись передаваемых данных в буфер rav_Buf_Txd.
;****************************************************
            ldi    YL,low(varBuf_Txd); Load Y register low буфер передачи
            ldi    YH,high(varBuf_Txd) ; Load Y register high буфер передачи
            clr    c_sumTRAN
...
;осуществляет чтение из буфера rav_Buf_Txd и запись в буфер varBuf_disp, начиная с четвертого адреса. 
;****************************************************            
            ldi    YL,low(varBuf_Txd); Load Y register low буфер передачи
            ldi    YH,high(varBuf_Txd) ; Load Y register high буфер передачи
...
;При каждом вызове подпрограммы осуществляется инкрементирование регистра Time. 
;Когда регистр достигнет значения VAL_time = 35 осуществляется сброс флага Disab_Key, чем разрешается отрос кнопки "Просмотр".
;*********************************************
...
REC_date: push   temp_L
          push   temp_H 
          in     temp_L,SREG
          push   temp_L;загружаем данные из temp_L, temp_H и SREG в СТЕК
;
		  in    temp_H,UCSRA;сохраняем регистр UCSRA
rd_UDR:	  in    temp_L,UDR;сохраняем в регистр принятый байт
		  rjmp   rt_rec; поппрограмма проверки принятого байта
;
pop_rec: 
          pop    temp_L
		  out    SREG,temp_L
...
;***********************************
rt_rec: 
          andi   temp_H,(1<<FE)|(1<<DOR)|(1<<PE); проверка флагов ошибок
          breq   USART2NoError
          rjmp   pop_rec; если есть ошибки приема, то не запоминаем байт
USART2NoError:            
		  st     Y+,temp_L; если нет ошибок приема, то запоминаем байт в буфер данных
		  inc    Cou_Rec; увеличиваем число принятых байт
		  mov    temp_H,Cou_Rec
		  cpi    temp_H,0x01
		  breq   rec_1_b; проверка на первый байт(перейдем,если первый)
		  cpi    temp_H,VAL_REC
		  breq   rec_end; проверка на первый байт(перейдем,если последний)
		  add    c_sumREC,temp_L; прибавляем в контрольную сумму принятый байт
		  rjmp   pop_rec
rec_1_b:  add    c_sumREC,temp_L; прибавляем в контрольную сумму принятый байт
;запускается таймер Т1 для формирования времени таймаута принимаемых данных и разрешается прерывание по совпадению канала А таймера Т1
          in     temp_L,TIMSK;
		  set
...
TRANdate: push   temp_L
          ...
          push   temp_L
		  push   temp_H
          rjmp   r_trans;Осуществляется запись байта данных из буфера rav_Buf_Txd в регистр передачи
;
pop_tran:
		  pop    temp_H
		  pop    temp_L
...
r_trans:  inc    Cou_Tran
		  mov    temp_L,Cou_Tran
		  cpi    temp_L,VAL_TR 
          breq   end_tr;Если значение регистра Cou_Tran стало равно VAL_TR, то передача прекращается
          ld     temp_L,Y+
          out    UDR,temp_L
		  rjmp   pop_tran
...
;**********************************
;останавливается работа таймера Т1 и запрещается прервынае при совпадении канала А таймера Т1.
; Затем сбрасываются регистры Cou_Rec и c_sumREC, что приводит к тому, что данные, 
;принятые модулем USART будут записываться с начала буфера rav_Buf_Rxd.
