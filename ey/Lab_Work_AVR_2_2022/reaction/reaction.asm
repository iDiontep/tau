...
.def Hundreds   =R18;адреса символов для семисегментных индикаторов по таблице перекодировки
.def Tens       =R19;
.def Ones       =R20;
...
.def tempH      =R21;преобразованое  значение, хранящегося в регистрах TimeH:TimeL, в миллисекунды
.def tempH2     =R26;
.def temp2      =R27;
;*****************
...
Init_C:	  ser   temp;  (PC0-PC7) - выходы, задающие значение на семисегментном индикаторе
	      out   DDRC,temp
;
Init_D:    ldi   temp,0b11111001;PD1,PD2-входы, остальные-выходы (Кнопка "Готовность" PD1,кнопка "Реакция" PD2(INT0))
	      out   DDRD,temp
...
Start:    rcall  Display; вызов процедуры динамической индикации
          sbrc   Byte_fl,F_ready; проверяется состояние флага F_ready (если бит1=0 пропускаем start) устанавливается после нажатия готов
...
;==================================================   
Display:  dec   Disp_Count; уменьшаем счетчик обновления дисплея
;Индикатор активен, пока регистр Disp_Count уменьшается от значения Val_dispCount до 0, затем происходит смена активного индикатора
          brne  ex_displ; проверка нуля Z =0 (если верно, то переход)
;
          ldi   Disp_Count,Val_dispCount; загружаем в регистр время вкл индикатора
;          
          inc   Disp_Numb;выбираем следующий дисплей (от 0 до 2)
		  cpi   Disp_Numb,3
...
;
           ...
           rjmp   Cheat;если 0 (не горит), то кнопка "Реакция" была нажата преждевременно и переходим в Cheat
           clr    temp
